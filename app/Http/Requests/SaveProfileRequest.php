...
class SaveProfileRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'email' => ['required', 'string', 'lowercase', 'email', 'max:255', Rule::unique('customers')->ignore(auth('web')->user())],
            'username' => ['required', 'string', 'lowercase', 'max:255', Rule::unique('customers')->ignore(auth('web')->user())],
            'password' => ['nullable', 'confirmed', Password::defaults(), 'min:8', 'string'],
        ];
    }
